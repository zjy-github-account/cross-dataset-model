...
class ShallowConvNet(nn.Module):
    def __init__(
        ...
        self.net = nn.Sequential(
            ...
            Conv2dWithConstraint(F1, F2, (channel_num, 1), bias=False, max_norm=2),
            nn.BatchNorm2d(F2),
            ActSquare(),
            ...
            nn.Dropout(drop_out),
            nn.Flatten(),
            LinearWithConstraint(2440, num_classes, max_norm=0.5)
        )
...
class EEGNet(nn.Module):
    def initialBlocks(self, dropoutP, *args, **kwargs):
        ...
        block2 = nn.Sequential(
            nn.Conv2d(self.F1 * self.D, self.F1 * self.D, (1, 8),
                      padding=(0, 8 // 2), bias=False,
